...
\begin{frame}{9.1 A 记录与 CNAME 记录}{一个指向 IP，一个指向另一个主机名}
...
\begin{frame}[fragile]{9.2 用 dig 验证 DNS}{同时检查 Host Header 与站点匹配}
...
\begin{frame}{9.3 一个域名如何承载前后端}{推荐同域 + 路径分流}
...
\begin{frame}[fragile]{9.4 代理头传递外部请求信息}{协议、域名与客户端 IP}
  \begin{lstlisting}[language=bash]
...
\begin{frame}[fragile]{9.5 WebSocket 与 SSE 代理配置}{AI 流式输出的超时与缓冲}
  \begin{lstlisting}[language=bash]
# WebSocket
...
\begin{frame}{9.6 Let's Encrypt / ACME 自动签发}{验证域名控制权并定期续期}
  \begin{columns}[T]
    ...
    \begin{column}{0.52\textwidth}
      \centering
      \DAframed{\includegraphics[width=65mm]{assets/official/letsencrypt-certificate.png}}
    \end{column}
  \end{columns}
  \DAsource{Let's Encrypt · How It Works}{https://letsencrypt.org/how-it-works/}
\end{frame}

\begin{frame}{9.7 HTTP-01 与 DNS-01 怎么选}{课堂单域名优先 HTTP-01}
...
\begin{frame}{9.8 宝塔申请与部署 SSL}{站点设置中完成申请、安装与续签}
  \begin{columns}[T]
    \begin{column}{0.50\textwidth}
      \centering
      \DAframed{\includegraphics[height=39mm,trim=700 100 700 100,clip]{assets/official/bt-ssl.jpg}}
    \end{column}
...
\begin{frame}[fragile]{9.9 HTTP 跳转 HTTPS}{80 端口保留验证与重定向}
  \begin{lstlisting}[language=bash]
server {
    listen 80;
...
\begin{frame}{9.10 Mixed Content 排错}{HTTPS 页面中的 HTTP 请求会被拦截}
...
\begin{frame}{9.11 CORS 配置}{浏览器对跨 Origin 请求的约束}
...
\begin{frame}[fragile]{9.12 上线后验证域名与证书}{检查跳转、证书、接口和安全头}
  \begin{lstlisting}[language=bash]
...
\begin{frame}{9.13 证书续签与 HSTS}{自动续签、到期提醒与安全策略}
  \begin{itemize}
