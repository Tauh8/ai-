...
\begin{frame}{9.2 日志分层看什么}{同一请求最好能用 Request ID 串起来}
...
\begin{frame}[fragile]{9.3 502 Bad Gateway 决策树}{Nginx 到上游的连接失败}
...
\begin{frame}{9.4 404 有三种完全不同的来源}{先看响应内容和 Server Header}
...
\begin{frame}{9.5 数据库连接失败怎么分层}{网络、账号、Schema、迁移}
  \begin{enumerate}
...
\begin{frame}[fragile]{9.6 服务器资源不足的症状}{构建时和运行时的瓶颈不同}
  \begin{lstlisting}[language=bash]
free -h
df -h
...
\begin{frame}{9.7 备份范围与恢复验证}{数据库、上传文件、配置与证书}
...
\begin{frame}{9.8 单机部署的安全基线}{默认拒绝，按需开放}
  \begin{columns}[T]
...
\begin{frame}{9.9 一次规范发布包含哪些阶段}{发布前、中、后都要有证据}
...
\begin{frame}{9.10 回滚触发条件}{提前定义阈值与决策人}
  \begin{itemize}
...
\begin{frame}{9.11 从手工部署走向 CI/CD}{今天先把手工 SOP 做对，再自动化}
  \centering
  \begin{tikzpicture}[node distance=2mm]
...
\begin{frame}{9.12 课堂综合实战}{把 Day 3 / Day 4 项目真正交付给另一个人访问}
  \begin{enumerate}
...
\begin{frame}{9.13 官方参考资料 · 协作与运行时}
  \scriptsize
  \begin{itemize}
...
\begin{frame}{9.14 官方参考资料 · 宝塔、网络与 HTTPS}
  \scriptsize
  \begin{itemize}
